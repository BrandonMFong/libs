	...
	virtual int sort(VectorSort type = kVectorSortMerge) {
		switch (type) {
			case kVectorSortBubble:
				return Array::sortBubble(*this);
			case kVectorSortInsertion:
				return Array::sortInsertion(*this);
			case kVectorSortSelection:
				return Array::sortSelection(*this);
			case kVectorSortQuick:
				return Array::sortQuick(*this);
			case kVectorSortMerge:
			default:
				return Array::sortMerge(*this);
		}
	}
...
private:
	...
	static int sortInsertion(Array & c) {
		int i, j;
		T key;
		int n = c.size();
		for (i = 1; i < n; i++) {
	...
	static int sortSelection(Array & c) {
		int i, j, min_idx;
		int n = c.size();
	...
	static int sortQuick(Array & c) {
		return sortQuick(c, 0, c.size() - 1);
	}

	static int sortQuick(Array & c, int low, int high) {
		if (low < high) {
	...
	static int sortQuickPartition(Array & c, int low, int high) {
		// Choose the pivot
		T pivot = c[high];
	...
	static int sortMerge(Array & c) {
		return sortMerge(c, 0, c.size() - 1);
	}

	static int sortMerge(Array & c, S left, S right) {
		if (left >= right)
			return 0;
